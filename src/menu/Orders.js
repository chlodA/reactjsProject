...
function Orders() {
    ...
    const removeOrder = async (id) => {
        try {
            firebase.firestore().collection('orders').doc(id).delete().then((handleClose) => {
                ...
                setOpen(false);
                alert('Order removed from database')
            });
        } catch (error) {
            alert(error);
        }
    ...
    const updateStatus = async (id) => {
        try {
            firebase.firestore().collection('orders').doc(id).update({status: 'pending'}).then((res) => {
                console.log('Item moved to Package arived at address.');
    ...
    return (
        ...
        <div>
            ...
            <Box mb={10}>
                {listItem.map((item) => (
                    console.log(item.id + " " + item.shopname),
                        <Box m={2}>
                                        ...
                                        <ListItem key={item.id}>
                                            ...
                                            <Grid item xs={12} sm container>
                                                <Grid item xs={12} container direction="column">
                                                    <Grid item>
                                                        ...
                                                        <Typography component={'div'} variant="body2"
                                                            ...
                                                            <ListItemText
                                                                key={item.country}>{item.country}</ListItemText>
                                                        </Typography>
                                                    </Grid>
                                                </Grid>
                                                <Grid item className={classes.display}>
                                                    ...
                                                    >
                                                        <DialogTitle
                                                            id="alert-dialog-title">{'Delete Order'}</DialogTitle>
                                                        <DialogContent>
                                                    ...
                                                    <IconButton edge="end"
                                                                aria-label="edit"
                                                    >
                                                        <Link aria-label="edit" to={"/edit-order/" + item.id}>
                                                            <EditIcon/>
                                                        </Link>
                                                    </IconButton>

                                                    <ReactWhatsapp aria-label="whatsapp-button" number="" message=""
